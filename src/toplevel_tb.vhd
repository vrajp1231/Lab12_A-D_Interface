...
stimuli : process
    -- You can adjust this if you later determine the exact number
    -- of PSINCs for 360° (from simulation measurements).
    constant NUM_PS_STEPS : integer := 448;  -- ~ 360° at ~17.9 ps/step for 8 ns period
begin
    -- Initial conditions
    PSEN <= '0';

    -- Let clocks settle for a bit
    wait for 10 us;

    -- Rotate the ADC clock one full turn (approx.) using PSINC pulses.
    -- PSINCDEC is tied to '1' in the AD9467_INTERFACE instantiation (increment).
    for k in 0 to NUM_PS_STEPS - 1 loop
        -- Issue one "proper" phase-increment transaction:
        PSEN <= '1';
        wait until PSDONE = '1';  -- clock wiz completed this PSINC
        PSEN <= '0';
        wait until PSDONE = '0';  -- ready for next
        -- Optional: small idle time between steps so you can see each change clearly
        wait for 100 ns;
    end loop;

    -- After we've rotated all the way around, just stop.
 end process;
